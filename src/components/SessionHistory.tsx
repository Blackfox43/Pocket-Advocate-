...
export default function SessionHistory({
  ...
  const filteredSessions = sessions.filter((session) => {
    ...
    return (
      ...
      (session.opponentName?.toLowerCase() || "").includes(query) ||
        ...
        {showClearConfirm ? (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-2 py-0.5 text-[10px] font-mono rounded bg-red-500/20 text-red-300 hover:bg-red-500/30 transition-colors cursor-pointer"
            >
              Yes
            </button>
            <button
              onClick={() => setShowClearConfirm(false)}
              className="px-2 py-0.5 text-[10px] font-mono rounded bg-slate-800 text-slate-400 hover:text-white transition-colors cursor-pointer"
            >
              No
            </button>
          </div>
          ...
          <button
            ...
            className="text-[10px] font-mono text-slate-500 hover:text-red-400 transition-colors duration-150 cursor-pointer"
          >
            CLEAR ALL
          </button>
        )}
      ...
      {sessions.length > 3 && (
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-slate-950/80 border border-slate-800/80 focus:border-indigo-500/50 text-white rounded-xl py-1.5 pl-8 pr-3 text-[11px] placeholder:text-slate-600 outline-none transition"
          />
        </div>
      )}
            ...
            return (
              <div
                ...
                className={`group flex items-center justify-between p-3 rounded-xl border transition-all duration-150 ${
                  isSelected
                    ? "border-indigo-500/80 bg-indigo-950/25 shadow-sm shadow-indigo-500/10"
